...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $getEntityDetails = new Entitymst;
    ...
    {
        $getEntityDetails = $this->getEntityDetails($id,0);
    ...
    public function create(Request $request)
    {
        ...
        $getEntityDetails = $this->getEntityDetails($lastId,0);
        $getUserDetails = EntityResource::collection($getEntityDetails);
    ...
    public function update(Request $request,$id){
        ...
        $checkEntityUser = $this->getEntityDetails($id,1);
        if(empty($checkEntityUser)){
            return $this->errorResponse('User not found', 400);
        }

        $userEntityType = $checkEntityUser->entity_type;
        // Validation section
        $rules = [];
        $messages = [];
        $rules = [
        ...
        if ($request->has('role_id') && $userEntityType != 2) {
            $rules['role_id'] = 'required|numeric';
            $messages['role_id.required'] = 'Please enter role';
            $messages['role_id.numeric'] = 'Role value must be numeric';
        }
    ...
    public function destroy($id){
        ...
        // Delete entity
        $checkEntityData = $checkEntityUser;
        if (!empty($checkEntityData)) {
            $checkEntityData->deleted_by = $getAdminDetails->id;
            $checkEntityData->deleted_at = CommonHelper::getUTCDateTime(date('Y-m-d H:i:s'));
            $checkEntityData->deleted_ip = CommonHelper::getUserIp();
            $checkEntityData->update();
    ...
    public function getEntityDetails($id,$type){
        
        $getEntityDetails = Entitymst::where('id',$id);
        if($type == 1){
            $getEntityDetails = $getEntityDetails->first();
        }else{
            $getEntityDetails = $getEntityDetails->get();
        }
